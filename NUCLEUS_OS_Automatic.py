...
def generate_automatic_radiation_readings(conn, cycle_number):
    ...
    # Demonstration events make the automatic alert system easy to show.
    # These are project simulations, not real plant limits or measurements.
    if cycle_number % 6 == 0:
        readings[4][2] = round(random.uniform(1.05, 1.60), 2)   # Warning
    if cycle_number % 10 == 0:
        readings[5][2] = round(random.uniform(2.05, 2.35), 2)   # High
    if cycle_number % 15 == 0:
        readings[5][2] = round(random.uniform(2.60, 3.20), 2)   # Critical alert
...
def automatic_radiation_monitoring(conn):
    ...
    print("\nPress Ctrl+C to stop automatic monitoring and return to the menu.")
    ...
    try:
        while True:
            ...
            print("\nNext automatic reading in 5 seconds... (Ctrl+C to stop)")
            time.sleep(5)

    except KeyboardInterrupt:
        print("\n\nAutomatic radiation monitoring stopped.")
        print("Returning to Radiation Monitoring menu...")
    except Error as e:
